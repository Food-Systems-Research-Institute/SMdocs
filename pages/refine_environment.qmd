...
#| fig-align: center
get_dimension_ggraph(
  'data/trees/env_meeting_tree.csv',
  'Environment',
  y_limits = c(-1.5, 2.1)
)
```
...
prop_prob_or_must_include <- props %>% 
  ...
  group_by(indicator) %>% 
  summarize(prop_include = sum(freq) / 5) %>% 
...
prop_must_include <- props %>% 
  ...
  group_by(indicator) %>% 
  summarize(prop_must = sum(freq) / 5) %>% 
...
idx_scores_table <- idx_scores %>%
 full_join(prop_must_include) %>%
 full_join(prop_prob_or_must_include) %>%
 mutate(across(c(3:4), ~ ifelse(str_detect(index, 'Carbon'), 1, .x))) %>%
 arrange(desc(score)) %>%
 mutate(
   across(where(is.numeric), ~ ifelse(is.na(.x), 0, .x)),
   across(c(3:4), ~ format(round(.x, 2), nsmall = 2))
