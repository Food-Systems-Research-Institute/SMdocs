...
lm_figure <- output_data %>% 
  ...
  scale_fill_manual(
    values = dp_palette,
    name = "Dimension",
    labels = snakecase::to_title_case(names(dp_palette))
  ) +
  ...
  guides(
    ...
    fill = guide_legend(
      override.aes = list(shape = 22, size = 4, alpha = 1, color = dp_palette),
      order = 2,
      nrow = 2
    )
  ) +
  ...
  scale_y_discrete(
    labels = function(x) {
      sapply(x, function(y) {
        ...
        dimension_label_color <- dp_palette[as.character(dim_val)]
