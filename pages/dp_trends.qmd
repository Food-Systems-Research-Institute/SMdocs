...
get_tiny_plots <- function(df, vars, lwd = 0.75) {
  map(vars, ~ {
    df <- df %>%
...
county_plots <- get_tiny_plots(
  df = SMdocs::dp_metrics_county,
  vars = series_vars
)
state_plots <- get_tiny_plots(
  df = SMdocs::dp_metrics_state,
...
## Function to process data for figure

prepare_figure_data <- function(data) {
...
pct_change_graph <- function(data, dp_text_palette, label_dimension_lookup) {
...
# Processing data and making spatial regression figure
processed_sar_data <- prepare_figure_data(sar_framed_outputs)

errorsar_figure <- pct_change_graph(
  data = processed_sar_data$data,
  dp_text_palette = dp_text_palette,
  label_dimension_lookup = processed_sar_data$label_dimension_lookup
)
...
print(errorsar_figure)


## Testing function on lm_figure
processed_lm_data <- prepare_figure_data(framed_outputs)

lm_test_figure <- pct_change_graph(
  data = processed_lm_data$data,
  dp_text_palette = dp_text_palette,
