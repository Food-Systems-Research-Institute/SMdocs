...
```{r}
#| label: tiny_trend_graphs
#| include: false
# Saving tiny graphs of each for table in paper
plots <- map(series_vars, ~ {
  df <- datasets$dp_metrics_county %>%
    filter(variable_name == .x) %>%
    ggplot(aes(x = year, y = value)) +
    ...
    # scale_x_continuous(breaks = seq(2000, 2025, 25)) +
...
```{r}
#| label: tiny_trend_graphs_state
#| include: false
tiny_state_plots <- map(state_series_vars, ~ {
  df <- datasets$dp_metrics_state %>%
    filter(variable_name == .x) %>%
    ggplot(aes(x = year, y = value)) +
    geom_line(
  ...
  setNames(c(state_series_vars))

# Save them all to outputs with variable names
iwalk(tiny_state_plots, \(plot, var) {
  ggsave(
    filename = paste0('outputs/trend_plots/fig_trend_', var, '.png'),
    plot = plot,
    width = 2,
