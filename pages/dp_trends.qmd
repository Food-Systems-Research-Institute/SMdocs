...
dat <- SMdocs::dp_metrics_county_adj %>% 
  ...
  left_join(state_link, by = dplyr::join_by(link == fips)) %>% 
  select(-link) %>% 
...
# Define things for global env for parallel workers
min_max <- SMdocs::min_max
dp_weights <- SMdocs::dp_weights
neast_counties_2024 <- SMdata::neast_counties_2024
...
out <- future_map(vars, \(var) {
  ...
  tryCatch({
    ...
    df <- dat %>% 
      dplyr::select(fips, year, !!dplyr::sym(var), state_code) %>% 
      tidyr::drop_na(!!dplyr::sym(var)) %>% 
      dplyr::mutate(normed = min_max(!!dplyr::sym(var)))
    ...
    if (!is.na(weight_var) && length(weight_var) > 0) {
      weight_df <- dp_weights %>% 
        ...
        dplyr::filter(weight_name == weight_var, stringr::str_length(fips) == 5)
      
      # Join weights back to df
      if (all(is.na(weight_df$weight_year))) {
        df <- df %>% 
          dplyr::inner_join(weight_df, by = dplyr::join_by(fips))
      } else {
        df <- df %>% 
          dplyr::inner_join(weight_df, by = dplyr::join_by(fips, year == weight_year))
      }
...
aics <- imap(sar_outs, ~ {
  df <- data.frame(
    model = .y,
    aic_spatial = AIC(.x),
    aic_lm = .x$AIC_lm.model
  ) %>% 
    mutate(
      diff = aic_spatial - aic_lm, # negatives mean spatial is better
      outcome = ifelse(diff < 0, 'spatial', 'lm')
    )
}) %>% 
  bind_rows()
# percentage that are better as spatial
(perc_spatial <- round(mean(aics$outcome == 'spatial') * 100, 1))

# Start putting together clean DF for reporting
...
`{r} perc_spatial`% of metrics have lower AIC in spatial model than regular OLS.
...
Proportions of metrics that are getting better, worse, or not changing, broken down by dimension:

```{r}
#| label: metrics_trends_by_dimension
sar_framed_outputs %>% 
  group_by(dimension, outcome) %>% 
  summarize(count = n()) %>% 
  ungroup() %>% 
  group_by(dimension) %>% 
  mutate(total = sum(count)) %>% 
  ungroup() %>% 
  mutate(percent = format(round(count / total * 100, 1), nsmall = 1)) %>% 
  select(-total) %>% 
  get_reactable()
```
