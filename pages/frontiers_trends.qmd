...
Run spatial regression for all vars. Might be worth looking into whether there is a version of Moral test that can be run across years. Until then, just running each one with spatiall error model. 
...
out <- future_map(vars, \(var) {
  ...
  tryCatch({
    ...
    # # 1. Check for different lengths. Remove CT if so
    # check <- df %>% 
    #   group_by(fips) %>% 
    #   summarize(count = n()) %>% 
    #   pull(count) %>% 
    ...
    # 
    # if (length(check) != 1) {
    #   df <- filter(df, str_detect(fips, '^09', negate = TRUE))
    #   ct_note <- 'removed'
    #   cat('\nRemoved Connecticut')
    # } else {
    #   ct_note <- 'included'
...
Still have to do the thing to figure out whether these are getting better or worse. This just shows which ones are changing over time.