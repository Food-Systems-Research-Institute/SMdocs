...
lm1 <- lm(
  foodInsecurity ~ economics + environment + health + production + social + gdp_per_cap,
  data = dat
)
```
...
#| label: life_exp
lm2 <- lm(
  lifeExpectancy ~ economics + environment + health + production + social + gdp_per_cap,
  data = dat
)
```
...
#| class: centered-table
#| echo: FALSE
tab_model(lm2, digits = 3, vcov.fun = life_exp_vcov)
```
...
#| label: happiness
lm4 <- lm(
  happinessScore ~ economics + environment + health + production + social + gdp_per_cap,
  data = dat
)
```
