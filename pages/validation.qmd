...
# Function for tab_model settings
my_tab_model <- function(model,
                         ...
                         dv.labels = c(
                           'Dimensions Only', 
                           'With GDP per Capita',
                           'With GDP and Population Weights'
                          ),
                         ...) {
  ...
  tab_model(
    ...
    # CSS = list(
      ...
      # css.depvarhead = 'border-bottom: 2px solid black; width: 100%;'
      # css.th = 'border-bottom: 2px solid black;',
      # css.td = 'border-top: 2px solid black;'
...
#| echo: FALSE
my_tab_model(lm1, title = 'Food Insecurity Index')
```
...
#| class: centered-table
#| echo: FALSE
my_tab_model(lm2, title = 'Life Expectancy') + theme_538()
```
...
#| class: centered-table
#| echo: FALSE
my_tab_model(lm2, title = 'Life Expectancy - Robust SEs', vcov.fun = life_exp_vcov)
```

When using robust errors, the health dimension falls off and we are left with only GDP per capita as a significant predictor of life expectancy. That being said, the model is explaining `r summary(lm2)$adj.r.squared * 100`% of the variance in life expectancy, which is surprisingly strong.
...
#| class: centered-table
#| echo: FALSE
my_tab_model(lm3, title = 'Food Environment Index')
```
...
#| class: centered-table
#| echo: FALSE
my_tab_model(lm4, title = 'Happiness Index')
```
