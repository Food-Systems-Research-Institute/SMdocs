...
# Save and read the merged data
write.csv(merged_df, "metrics_and_meta.csv", row.names = FALSE)

metrics_and_meta <- readr::read_csv("metrics_and_meta.csv")

# Group dimension columns together in ascending order; filter out before 2000 and NAs
...
# Plot
ggplot(data_full, aes(x = as.factor(year), y = metric, fill = dimension)) +
  geom_tile(color = "gray80", linewidth = 0.25) +
  scale_y_discrete(labels = to_title_case) +
  scale_x_discrete(
    breaks = seq(min(heatmap_easy_data$year), max(heatmap_easy_data$year), by = 5)) +
