...
Throwing those same variables into a correlation matrix. Hover to see variable names, Pearson correlationm, and p-values.
...
PCA is a popular tool in this area for explore unique variation with redundant variables. But it requires complete data, which we do not have. So I'm using missForest to impute here. This really warrants a deeper dive into the type and severity of missingness, but I'm just going to run with it for now.
...
From the scree plot and eigenvalues it looks like the first three composites bring lots of variance, but after that there is no clear elbow. Composite #2 looks like it is influenced by population more than anything, as the variables that are raw counts load the heaviest onto it (number employed, number of farmers markets, etc). Worth noting that it is probably best to use relative metrics here rather than raw numbers. Composite #3 has a curious collection of median earnings variables and per farm variables like acres, income per farm. 