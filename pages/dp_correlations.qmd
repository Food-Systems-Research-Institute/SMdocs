...
colored_labels <- ordered_framework %>%
  mutate(
    label = paste0(
      "<span style='color:", dp_text_palette[dimension], "'>", to_title_case(str_replace_all(metric, "_", " ")), "</span>"
    )
  ) %>%
  select(metric, label) %>%
  deframe()
...
corplot <- ggplot(cor_r, aes(x = var_1, y = var_2)) +
  ...
  geom_tile(data = cor_r %>% filter(!is.na(value) & significant), aes(fill = value)) +
  
  # Make non-significant correlations white
  geom_tile(data = cor_r %>% filter(!is.na(value) & !significant), fill = "white") +
  ...
  scale_x_discrete(labels = colored_labels, limits = metric_order) +
  scale_y_discrete(labels = colored_labels, limits = reversed_metric_order) +
  ...
  theme(
    ...
    legend.title = element_text(size = 10, family = "Times New Roman", hjust = 1),
...
df <- df %>% 
  ...
  mutate(
    ...
    r = ifelse(sig == '*', paste0(r, '*'), r)
  ) %>% 
  select(-c(sig, p))
get_str(df)

# Round and add stars for significance
df <- df %>%
  pivot_wider(
    names_from = var_2,
    values_from = r,
...
df <- df %>% 
  mutate(var_1 = str_sub(var_1, end = 20)) %>%
  mutate(across(!var_1, ~ ifelse(is.na(.x), '', .x))) %>% 
  setNames(c(names(.) %>% str_sub(end = 20)))
get_str(df)
colnames(df)[1] <- ' '
get_str(df)
...
df %>% 
  kbl(
    format = "latex",
    caption = 'Correlation matrix of county level metrics using latest available time point',
  ...
  footnote(
    general_title = "",
    general = 
      '\\\\textit{Note: }*: p < 0.05. Connecticut is not included in correlation analyses.',
    threeparttable = TRUE,
    escape = FALSE
  ) %>% 
  save_kable(
