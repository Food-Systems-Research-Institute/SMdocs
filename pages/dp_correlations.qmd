...
colored_labels <- ordered_framework %>%
  mutate(
    label = paste0(
      "<span style='color:", dimension_colors[dimension], "'>", to_title_case(str_replace_all(metric, "_", " ")), "</span>"
    )
  ) %>%
  select(metric, label) %>%
  deframe()
...
dimension_legend_df <- tibble(
  x = -Inf,
  y = -Inf,
  dimension = factor(names(dimension_colors), levels = names(dimension_colors))
)
...
corplot <- ggplot(cor_r, aes(x = var_1, y = var_2, fill = value)) +
  ...
  scale_color_manual(
    ...
    labels = str_to_title(names(dimension_colors)),
    name = "Dimension"
  ) +
