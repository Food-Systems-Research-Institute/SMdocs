...
tmap_save(
  tm = map,
  filename = 'temp/conferences/c3_map.png',
  asp = 0,
  dpi = 300
)
...
states <- SMdata::neast_states
mapview(states)

# Get state shapefiles for New England + North Carolina
ne_fips <- SMdata::fips_key %>%
  filter(!is.na(state_code) & fips != '00')

states <- tigris::states(year = 2024) %>%
  filter(STUSPS %in% c(ne_fips$state_code, 'NC'))
mapview(states)

tmap_mode('plot')

# Iterations: Vermont, NE, NE + NC
iters <- c('VT', 'NE', 'NE_plus_NC')
...
walk(iters, ~ {
  if (.x == 'VT') {
    state_iter = filter(states, STUSPS == 'VT')
  } else if (.x == 'NE') {
    state_iter = filter(states, STUSPS != 'NC')
  } else if (.x == 'NE_plus_NC') {
    state_iter = states
  }
  ...
  tmap_save(
    tm = map,
    filename = paste0('temp/synthesis/', .x, '.png'),
    asp = 0,
    dpi = 300
  )
})
