...
# Consider spatial locations of statinos, pull form above
...
# Get covariance matrices for different lags
Lag0_cov <- cov(X, use = 'complete.obs')
Lag1_cov <- cov(X[-1, ], X[-nrow(X),], use = 'complete.obs')


# Split into four longitudinal strips to help interpret matrix?
...
E <- svd(Zt)
get_str(E)
# contains matrices V, U, and singular values diag(D)

V <- E$v
# V has EOFs in space-wide format
...
# Matrix U has principal component time series in wide format
dim(E$u)
E$u

# Each col is a time series associated with an EOF
...
# Now normalize the time series
TS$nPC <- TS$PC * sqrt(nT-1)
get_str(TS)
get_table(TS$t)
length(TS$EOF %>% unique)
# 400 EOFs - why
