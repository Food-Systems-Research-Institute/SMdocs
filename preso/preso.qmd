...
# 1. Framework and Methods {.center}
...
-   Allows for more robust than leaving outliers
...
-   Flexible and intuitive premise
-   Challenging to set targets in many applications
::::
...
-   Partially compensable - rewards even performance across areas
...
::: notes
-   We now have 10 combinations of scaling and aggregation.
-   Note that Z-score geometric transformation is hinky
-   TRANSITION INTO CASE STUDY OF MIN MAX GEO
:::

# 2. Case Study

## Case Study: Indicator Distributions

An example of indicator distributions with min-max scaling and geometric
aggregation
...
Using min-max scaling and geometric aggregation
...
-   Env: Nevada skews everything
-   Health: Mississippi skews everything
...
Min max scaling and geometric aggregation
...
-   Note: social connectedness is pretty high (social associations, disconnected youth, single parent households)
...
get_pca_table <- function(table) {
  table %>% 
    setNames(c(stringr::str_to_title(names(.)))) %>% 
    get_reactable(
      pagination = FALSE,
      defaultPageSize = 50,
...
::: notes
-   Scree plot: would say 6 components
...
-   VSS: compare fit of simplified model to original correlations. VSS =
    1-sumsquares(r\*)/sumsquares(r). Peaks at optimal (most interpretable)
    number of factors.
...
## Uncertainty: Metric Counts {.smaller}
...
ggplotly(
  uncertainty_plots$dimension_ranks,
  width = 700,
  height = 400,
  tooltip = 'text'
)
...
-   SD only goes down for health and economics - both have an indicator
    with many metrics
...
-   Steps:
    1. Run all 10 methods without the indicator
    2. Run all 10 methods with the indicator
...
-   (Format and add image on the right)
...
-   Indicator sensitivity across all states, not just Vermont?
-   Should I remove the part where I ditch 5 metrics that I can't put a direction on?
-   Should I remove or change the production species diversity metric?
